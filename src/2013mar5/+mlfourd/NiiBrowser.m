classdef NiiBrowser < mlfourd.NIfTI
    ...
    methods
        ...
        function this  = NiiBrowser(nii, blr, blk)
            ...
            assert(all(size(nii.mmppix) == size(this.blur)));
            assert(all(size(nii.mmppix) == size(this.block)));
        end % ctor   
        ...
        function this = blurredBrowser(this, blr, msk)
            ...
            this.fileprefix = NamingRegistry.averagedName(this.fileprefix);
    ...
    methods (Static)
        ...
        function niib  = load(fn, desc, blr, blk)
            ...
            switch (nargin)
                case 1
                    ...
                    blr  = PETBuilder.petPointSpread; blk = [1 1 1];
                case 2
                    blr  = PETBuilder.petPointSpread; blk = [1 1 1];
                case 3
                                                       blk = [1 1 1];
        ...
        function sz    = embedVecInSitu(sz, fixedsz)
            ...
            %        sz = 
            %            18 18 31
            assert(size(sz,1) == 1);  assert(size(fixedsz,1) == 1);
