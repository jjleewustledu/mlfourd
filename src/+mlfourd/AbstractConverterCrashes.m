classdef AbstractConverterCrashes < mlfourd.ConverterInterface
	%% AbstractConverterCrashes is the interface for concrete strategies such as SurferDicomConverter, MRIConverter
    ...
    %  Version $Revision: 2608 $ was created $Date: 2013-09-07 19:14:08 -0500 (Sat, 07 Sep 2013) $ by $Author: jjlee $,  
 	%  last modified $LastChangedDate: 2013-09-07 19:14:08 -0500 (Sat, 07 Sep 2013) $ and checked into svn repository $URL: file:///Users/jjlee/Library/SVNRepository_2012sep1/mpackages/mlfourd/src/+mlfourd/trunk/AbstractConverterCrashes.m $ 
 	%  Developed on Matlab 7.13.0.564 (R2011b) 
 	%  $Id: AbstractConverterCrashes.m 2608 2013-09-08 00:14:08Z jjlee $ 
    ...
    properties (Dependent)
        ...
        fslPath                       
        modalityFolder 
        modalityPath % rootpath for AbstractConverterCrashes objects 
        sessionId      
        sessionPath
        studyFolder
        studyPath
    ...
    methods (Static)
        ...
        function          convertStudy(studyPth, patt, varargin)
            ...
            for d = 1:length(dns)
                try
                    mlfourd.AbstractConverterCrashes.convertSession(dns{d}, varargin{:});
                catch ME
                    handwarning(ME);
                end
            end
        ...
        function this   = createFromSessionPath(sessionpth)
            ...
            import mlfourd.*;
            this = AbstractConverterCrashes.createFromModalityPath( ...
                firstExistingFile(sessionpth, AbstractConverterCrashes.modalityFolders));
        end
        function          orientRepair(obj, o2fix)
            %% ORIENTCHANGESTRATEGY flips image-objects along requested orientations
            %  Usage:  nifti = AbstractConverterCrashes.orientRepair(imgObj, o2fix)
            %                                                         ^ fileprefix, numerics, NIfTIInterface, cell-array
            %                                                                 ^ struct
            ...
            try
                if (iscell(obj))
                    for b = 1:length(obj)
                        AbstractConverterCrashes.orientRepair(obj{b}, o2fix)
                    end
                elseif (isa(obj, 'NIfTIInterface'))
                    AbstractConverterCrashes.reorientByNIfTIInterface(obj, o2fix);
                elseif (isnumeric(obj))
                    AbstractConverterCrashes.fixImageLeafOrientLeaf(NIfTI(obj), o2fix);
                elseif (ischar(obj))
                    AbstractConverterCrashes.reorientByLocation(obj, o2fix);
                else                
                    handwarning(ME1, ...
                        ['AbstractConverterCrashes.orientRepair failed to change ' char(obj) ' to ' char(o2fix)]);
                end
            catch ME
                handwarning(ME);
            end
        ...
        function fns    = brightest(fns)
            import mlfourd.*;
            fns = ImagingParser.brightest(AbstractConverterCrashes.ensureFilenames(fns));
        end
        function fn     = lowestSeriesNumber(fns)
            import mlfourd.*;
            fn = ImagingParser.lowestSeriesNumber(AbstractConverterCrashes.ensureFilename(fns));
        end
        function fns    = mostEntropy(fns)
            import mlfourd.*;
            fns = ImagingParser.mostEntropy(AbstractConverterCrashes.ensureFilenames(fns));
        end
        function fns    = mostNegentropy(fns)
            import mlfourd.*;
            fns = ImagingParser.mostNegentropy(AbstractConverterCrashes.ensureFilenames(fns));
        end
        function fns    = smallestVoxels(fns)
            import mlfourd.*;
            fns = ImagingParser.smallestVoxels(AbstractConverterCrashes.ensureFilenames(fns));
        end
        function fns    = longestDuration(fns)
            import mlfourd.*;
            fns = ImagingParser.longestDuration(AbstractConverterCrashes.ensureFilenames(fns));
        end
        function fns    = timeDependent(fns)
            import mlfourd.*;
            fns = ImagingParser.timeDependent(AbstractConverterCrashes.ensureFilenames(fns));
        end % static timeDependent
        function fns    = timeIndependent(fns)
            import mlfourd.*;
            fns = ImagingParser.timeIndependent(AbstractConverterCrashes.ensureFilenames(fns));
        end % static timeIndependent
        function fns    = notMcf(fns)
            import mlfourd.*;
            fns = FlirtBuilder.notMcf(AbstractConverterCrashes.ensureFilenames(fns));
        end % static notMcf
        function fns    = notFlirted(fns)
            import mlfourd.*;
            fns = FlirtBuilder.notFlirted(AbstractConverterCrashes.ensureFilenames(fns));
        end % static notFlirted
        function fns    = notBetted(fns)
            import mlfourd.*;
            fns = FlirtBuilder.notBetted(AbstractConverterCrashes.ensureFilenames(fns));
        end % static notBetted
        function fns    = isPet(fns)
            import mlfourd.*;
            fns = ImagingParser.isPet(AbstractConverterCrashes.ensureFilenames(fns));
        end % static isPet
        function fns    = notIsPet(fns)
            import mlfourd.*;
            fns = ImagingParser.notIsPet(AbstractConverterCrashes.ensureFilenames(fns));
        end % static notIsPet
        function          analyze2nifti(pth)
            %% ANALYZE2NIFTI
            %  Usage:  AbstractConverterCrashes.analyze2nifti([full_path])
    ...
    methods 
        function fns  = timeindFqFilename(this, lbl)
            import mlfourd.*;
            fns = AbstractConverterCrashes.mostNegentropy( ...
                  AbstractConverterCrashes.smallestVoxels( ...
                  AbstractConverterCrashes.timeIndependent(this.([lbl 'FqFilenames']))));            
        ...
        function fns  = timedepFqFilename(this, lbl)
            ...
            fns = AbstractConverterCrashes.mostNegentropy( ...
                  AbstractConverterCrashes.smallestVoxels( ...
                  AbstractConverterCrashes.timeDependent(this.([lbl 'FqFilenames']))));            
            assert(~isempty(fns), 'mlfourd:MissingValues', 'timeindFqFilename.fns is empty');
        end % timeindFqFilename        
        function        orientChange2Standard(this, fslpth)
            ...
            dt = mlsystesm.DirTool(fullfile(fslpth, '*.nii'));
        ...
        function        cleanAnalyze(this, pth)
            ...
            bakpth  = ensuredir(fullfile(pth, this.namingRegistry_.backupFolder));
    ...
    methods (Access = 'protected')
        function this  = AbstractConverterCrashes(modalPth)
            %% AbstractConverterCrashes
            %  Usage:  obj = AbstractConverterCrashes(modality_path)
        ...
        function fqfns = fqfnsInFslpath(this, patt)
            ...
            assert(ischar(patt));
            dt    = mlsystem.DirTool(fullfile(this.fslPath, [patt '*']));
    ...
    methods (Static) %, Access = 'private')
        function         ensureFloat(fp)
            ...
            try
                ...
                if (~lstrfind(datType, upper(AbstractConverterCrashes.ODT))) 
                    FslBuilder.fslmaths( ...
                        [fp ' ' AbstractConverterCrashes.tempFileprefix(fp) ' -odt ' lower(AbstractConverterCrashes.ODT)]);
                    AbstractConverterCrashes.moveTempBack(fp);
                end
            catch ME
                handwarning(ME);
            end
            ...
            try
                mlbash( ...
                    stringf('fslreorient2std %s %s',  ...
                             fp, AbstractConverterCrashes.tempFileprefix(fp)));
                AbstractConverterCrashes.moveTempBack(fp);
            catch ME
                handwarning(ME);
            end
        ...
        function [s,r] = moveTempBack(fp, exts)
            tfp = mlfourd.AbstractConverterCrashes.tempFileprefix(fp);
            if (~lexist('exts','var'))
                exts = '.nii.gz'; end
        ...
        function       reorientByNIfTIInterface(imgint, o2fix)
            import mlfourd.*;
            nii = NIfTI(imgint);
            AbstractConverterCrashes.fixImageLeafOrientLeaf(nii, o2fix);
        end
        function       reorientByLocation(fstr, o2fix)
            %% FIXORIENTBYLOCATION
            %  Uses:   AbstractConverterCrashes.reorientByLocation(path_to_files) 
            %                                                ^ or filename
            
            import mlfourd.*;
            fstr = filename(fstr, '.nii.gz');
            if (lexist(fstr, 'dir'))
                AbstractConverterCrashes.reorientInFolder(fstr, o2fix);
            elseif (lexist(fstr, 'file'))
                AbstractConverterCrashes.reorientFile(fstr, o2fix);
            else
                error('mlfourd:UnsupportedType', ...
                      'AbstractConverterCrashes.reorientByLocation.fstr->%s, class(fstr)->%s', char(fstr), class(fstr));
            end
        ...
        function       reorientInFolder(fld, o2fix)
            ...
            for d = 1:length(dt.fqfns) 
                AbstractConverterCrashes.reorientFile(dt.fqfns{d}, o2fix); 
            end
        end % static reorientInFolder
        function       reorientFile(fqfn, o2fix)
            import mlfourd.*;
            AbstractConverterCrashes.fixImageLeafOrientLeaf(NIfTI.load(fqfn), o2fix);
