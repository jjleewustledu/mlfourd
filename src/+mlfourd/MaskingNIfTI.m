classdef MaskingNIfTI < mlfourd.AbstractNIfTIDecorator
	%% MASKINGNIFTI is a NIfTIdecorator that composes an internal INIfTI object
    ...
    methods (Static)
        function this = load(varargin)
            import mlfourd.*;            
            this = MaskingNIfTI(NIfTIContext.load(varargin{:}));
        end
    ...
    methods
        function this = MaskingNIfTI(cmp, varargin)
            %% MASKINGNIFTI 
            ...
            %  @return mlfourd.MaskingNIfTI instance.
            %  @throw
            
            import mlfourd.*; 
            this = this@mlfourd.AbstractNIfTIDecorator(cmp);
            this = this.append_descrip('decorated by MaskingNIfTI');
        ...
        function this = binarized(this)
            ...
            if (dipsum(img) == numel(img))
                warning('mlfourd:possibleMaskingError', ...
                    'MaskingNIfTI.binarized mask spans the image space');
            end
            this.assertVolumeFraction;
            this.addLog('MaskingNIfTI.binarized');
        end
        ...
        function this = masked(this, msk)
            ...
            if (~this.isZeroToOne(msk))
                 warning('mlfourd:possibleMaskingError', ...
                         'MaskingNIfTI.masked received a mask with values outside of [0 1]');
            end
            this = this.makeSimilar( ...
            ...
            this.assertVolumeFraction;
            this.addLog('MaskingNIfTI.masked(%s)', char(msk));
        end
        function this = maskedByZ(this, rng)
            ...
            this = this.masked( ...
                mlfourd.NIfTIContext(zimg, 'fileprefix', sprintf('maskz%i-%i', ceil(rng(1)), floor(rng(2)))));
            warning('on',  'mlfourd:possibleMaskingError');
            this.addLog('MaskingNIfTI.maskedByZ(%s)', mat2str(rng));
        end
        function this = thresh(this, t)
            ...
            this.addLog('MaskingNIfTI.thresh(%g)', t);
        end
        function this = threshp(this, p)
            ...
            this.addLog('MaskingNIfTI.threshp(%g)', p);
        end          
        function this = threshPZ(this, p)
            ...
            this.addLog('MaskingNIfTI.threshPZ(%g)', p);
        end
        function this = uthresh(this, t)
            ...
            this.addLog('MaskingNIfTI.uthresh(%g)', t);
        end
        function this = uthreshp(this, p)
            ...
            this.addLog('MaskingNIfTI.uthreshp(%g)', p);
        end          
        function this = uthreshPZ(this, p)
            ...
            this.addLog('MaskingNIfTI.uthreshPZ(%g)', p);
        end
        function this = zoomed(this, zoom)
            ...
            offset = round(abs(size1 - size0)/2);
            img1 = zeros(size1);
            
            if (all(zoom >= 1))
               
                % zoom > 1, offset > 0, injective or 1-to-1; "enlarge canvas"
                if (3 == length(size0))
                    for x3 = 1:size0(3)
                        for x2 = 1:size0(2)
                            for x1 = 1:size0(1)
                                img1(x1+offset(1), x2+offset(2), x3+offset(3)) = this.img(x1, x2, x3);
                            end
                        end
                    end
                elseif (4 == length(size0))
                    for t = 1:size0(4)
                        for x3 = 1:size0(3)
                            for x2 = 1:size0(2)
                                for x1 = 1:size0(1)
                                    img1(x1+offset(1), x2+offset(2), x3+offset(3), t) = this.img(x1, x2, x3, t);
                                end
                            end
                        end
                    end
                else
                    error('mlfourd:unsupportedSize', 'MaskingNIfTI.zoom.size0->%s', num2str(size0));
                end
                
            elseif (all(zoom <= 1))

                % zoom < 1, offset < 0, surjective or onto; "crop"
                if (3 == length(size0))
                    for x3 = 1:size1(3)
                        for x2 = 1:size1(2)
                            for x1 = 1:size1(1)
                                img1(x1, x2, x3) = this.img(x1+offset(1), x2+offset(2), x3+offset(3));
                            end
                        end
                    end
                elseif (4 == length(size0))
                    for t = 1:size0(4)
                        for x3 = 1:size1(3)
                            for x2 = 1:size1(2)
                                for x1 = 1:size1(1)
                                    img1(x1, x2, x3, t) = this.img(x1+offset(1), x2+offset(2), x3+offset(3), t);
                                end
                            end
                        end
                    end
                ...
                error('mlfourdfp:unexpectedConditionalBranch', 'MaskingNIfTI.zoomed');
            end
            ...
            this.addLog('MaskingNIfTI.zoomed(%s)', char(this));
        end
    ...
    methods (Access = private)
        function assertVolumeFraction(this)
            ...
            if (volFrac > 0.5)
                warning('mlfourd:possibleMaskingError', ...
                        'MaskingNIfTI encountered a masked image with a large volume-fraction:  %g', ...
                        volFrac);
            end
        end
        function msk = resizeMaskToFit(this, msk)
            ...
            error('mlfourd:unsupportedImageSize', 'size(MaskingNIfTI.resizeMaskToFit.msk)->%s', mat2str(size(msk)));
        end
    end
