classdef MaskingTool_20211201 < handle & mlfourd.ImagingFormatTool_20211201
	%% MASKINGTOOL
    %
    ...
    methods
        function this = binarized(this)
            ...
            this.addLog('MaskingTool_20211201.binarized');
        end
        ...
        function this = imfill(this, varargin)
            ...
            this.addLog('MaskingTool_20211201.imfill');
        end
        function this = masked(this, M)
            ...
            switch (this.ndims)
                ...
                otherwise
                    error('mlfourd:unsupportedSwitchcase', 'MaskingTool_20211201.masked.ndims->%i', this.ndims);
            end
            this.warnLargeVolumeFraction;
            this.fileprefix = sprintf('%s%s_%s', this.fileprefix, this.MASKEDBY, M.fileprefix);
            this.addLog('MaskingTool_20211201.masked by %s', M.fileprefix);
        end
        function this = maskedMaths(this, M, funch, varargin)
            ...
            this.addLog('MaskingTool_20211201.maskedMaths %s %s', M.fileprefix, func2str(funch));
        end
        function this = maskedByZ(this, rng)
            ...
            this.addLog('MaskingTool_20211201.maskedByZ range %s', mat2str(rng));
        end
        ...
        function this = thresh(this, t)
            ...
            this = this.makeSimilar( ...
                   ...
                   'descrip',    sprintf('MaskingTool_20211201.thresh(%g)', t));
            this.addLog('MaskingTool_20211201.thresh(%g)', t);
        end
        function this = threshp(this, p)
            ...
            this.addLog('MaskingTool_20211201.threshp(%g)', p);
        end 
        function this = uthresh(this, t)
            ...
            this = this.makeSimilar( ...
                   ...
                   'descrip',    sprintf('MaskingTool_20211201.uthresh(%g)', t));
            this.addLog('MaskingTool_20211201.uthresh(%g)', t);
        end
        function this = uthreshp(this, p)
            ...
            this.addLog('MaskingTool_20211201.uthreshp(%g)', p);
        end   
        ...
        function this = MaskingTool_20211201(h, varargin)
    ...
    methods (Static, Access = private)
        ...
        function warnNonUnitMask(M)
            ...
            warning('mlfourd:possibleMaskingError', 'MaskingTool_20211201.warnNonUnitMask.M has max->%g', dipmax(mimg));
            histogram(mimg(mimg > 0), 100, 'Normalization', 'probability');
            xlabel('mask amplitude');
            ylabel('probability');
            title(sprintf('MaskingTool_20211201.warnNonUnitMask:  %s', M.fqfilename));
        end
    end
    ...
    methods (Access = private)
        ...
        function warnLargeVolumeFraction(this)
            ...
            if (volFrac > this.MAX_VOLUME_FRACTION)
                warning('mlfourd:possibleMaskingError', ...
                        'MaskingTool_20211201 encountered a masked image with a large volume-fraction:  %g', ...
                        volFrac);
            end
        end
    end
