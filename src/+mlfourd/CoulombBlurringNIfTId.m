classdef CoulombBlurringNIfTId < mlfourd.NIfTIdecoratorProperties
    ...
    methods (Static)
        ...
        function width = sigma2width(sigma, fheight)
            ...
            switch (nargin)
                ...
                case 2 
                otherwise
                    error('mlfourd:InputParamsErr', help('CoulombBlurringNIfTId.sigma2width'));
            end
        ...
        function sigma = width2sigma(width, fheight)
            ...
            switch (nargin)
                ...
                case 2
                otherwise
                    error('mlfourd:InputParamsErr', help('CoulombBlurringNIfTId.width2sigma'));
            end
	...
	methods
 		function this = CoulombBlurringNIfTId(varargin)
 			%% COULOMBBLURRINGNIFTID
    ...
    methods (Static, Access = private)
        function img   = chargesToScalarPotential(img, width, metric, metppix, height)
            ...
            %         img:      Gaussian-blurred image returned
            %  Examples:
            %         img = this.chargesToScalarPotential(img, [fwhh_x fwhh_y])
            %         img = this.chargesToScalarPotential(img, fwhh_vec3, 'mm', mlpet.PETBuilder.petPointSpread, 0.1)   
        end
        function img   = gaussFullwidth(img, width, metric, metppix, height)
            ...
            %                 numeric objects.
            %  Usage: gimg = CoulombBlurringNIfTId.gaussFullwidth(img, width, metric, metppix, height)
            %         img:       numeric object
            %         width:     row vector of full widths
            %         metric:    units of full width blur
            %         metppix:   metric units per pixel; 1 and [1 1 1] are equivalent
            ...
            %  See also:  CoulombBlurringNIfTId.gaussSigma
            ...
            switch (nargin)
                ...
                otherwise
                    error('mlfourd:NotImplementedErr', ...
                         ['CoulombBlurringNIfTId.gaussFullwidth.nargin->' num2str(nargin)]);
            end
            img = CoulombBlurringNIfTId.gaussSigma(img, CoulombBlurringNIfTId.width2sigma(width, height), metric, metppix);
        end 
        function img   = gaussSigma(img, sigma, metric, metppix)
            %% GAUSSSIGMA applies multi-dimensional, anisotropic, Gaussian filtering to 
            %             numeric objects.
            %  Usage: gimg = CoulombBlurringNIfTId.gaussSigma(img, width, metric, metppix)
            %         img:       numeric object
            ...
            switch (nargin)
                ...
                otherwise
                    error('mlfourd:NotImplementedErr', ...
                         ['CoulombBlurringNIfTId.gaussSigma.nargin->' num2str(nargin)]);
            end
            switch (lower(metric)) 
                ...
                otherwise
                    error('mlfourd:NotImplementedErr', ...
                         ['CoulombBlurringNIfTId.gaussFullwidth.metric->' metric ' was unrecognizable;\n' ...
                          'try pixel(s), voxel(s), mm, cm']);
            end
            img     = double(img);
            imgRank = length(size(img));
            if (length(sigma) < imgRank)
                sigma = CoulombBlurringNIfTId.embedVecInSitu(sigma, zeros(size(size(img))));
            end
            if (length(metppix) < length(sigma))
                metppix = CoulombBlurringNIfTId.stretchVec(metppix, length(sigma));
            ...
            h1  = reshape(CoulombBlurringNIfTId.gaussian(h0, zeros(1,imgRank), sigma), krnlLens);
            img = imfilter(img, h1);
