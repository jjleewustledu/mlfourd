classdef FourdfpTool < handle & mlfourd.ImagingFormatTool
    %% FOURDFPTOOL
    %  N.B.:  When adjustQOffsets or adjustSRows fail because ImagingInfo in superclasses are deficient,
    %  typically because of specifications of Analyze75Info, ImagingInfo.make_nii() will read both hdr and img
    %  from the filesystem to ensure that hdr information is consistent with NIfTI specifications.
    %
    ...
    methods (Static)
        function this = createFromImagingFormat(iform)
            ...
            if isa(iform, 'mlfourd.MghTool')
                [img_,hdr_] = mlfourd.FourdfpInfo.exportFreeSurferSpaceToFourdfp(iform.img, iform.hdr);
            else
                img_ = iform.img;
                hdr_ = iform.hdr;
            end
