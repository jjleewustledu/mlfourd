classdef NIfTIdecorator7 < mlfourd.INIfTI & mlio.IOInterface
    ...
    %  KLUDGE:  Matlab checks that dependent properties do not access stored values.  Multiple composed NIfTIdecorator7 
    ...
    %      In class 'mlfourd.NIfTIdecorator7', the get method for Dependent property 'filename' attempts to 
    %      access the stored property value.  Dependent properties don't store a value and can't be accessed
    %      from their get method.
    %     
    %      Error in mlfourd.NIfTIdecorator7/get.filename (line 66)
    %                  fn = this.component_.filename;
    %     
    %  Using NIfTIdecorator7/3/4/5 for each decorator subclass is a work-around.
    ...
    methods (Static)
        ...
        function this = load(varargin)
            ...
            %  Usage:  obj = NIfTIdecorator7.load(filename[, description]);
            
            import mlfourd.*;
            this = NIfTIdecorator7(NIfTId.load(varargin{:}));
        end
    end

    methods
 		function this = NIfTIdecorator7(varargin) 
            ...
            %  Usage:  obj = NIfTIdecorator7(INIfTI_object);
