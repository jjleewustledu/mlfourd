classdef ImagingList < mlpatterns.CellArrayList
    %% IMAGINGLIST A cell array realisation of the List ADT
    ...
    %  last modified $LastChangedDate: 2012-08-23 16:21:49 -0500 (Thu, 23 Aug 2012) $ and checked into svn repository $URL: file:///Users/jjlee/Library/SVNRepository_2012sep1/mpackages/mlfourd/src/+mlfourd/trunk/ImagingList.m $
    %  Developed on Matlab 7.14.0.739 (R2012a)
    %  $Id: ImagingList.m 1231 2012-08-23 21:21:49Z jjlee $
    ...
    % 2010-Jul-20: Methods modified to work with arrays of ImagingList.
    methods % Public Access
        % Constructor.
        function newObj = ImagingList()
            % Use an arbitrary initial list capacity and pre-allocate.
            newObj = newObj@mlpatterns.CellArrayList;
        end
        ...
        function add(varargin)
            ...
            if nargin == 3
                ...
                assert(isnumeric(loc) && isscalar(loc) && floor(loc)==loc && loc>0,...
                    'MATLAB:List:ImagingList','Location must be a positive scalar integer.');
            else
        ...
        function elts = get(obj,locs)
            ...
            assert(isnumeric(locs) && isvector(locs) && isequal(floor(locs),locs) && all(locs > 0),...
                'MATLAB:List:ImagingList','Locations must be vector of positive integers.');
        ...
        % 2010-Jul-20: If this obj is an array of ImagingLists then display
        % each in order.
        ...
        % Overloaded.  For string operations.
        function str = char(obj)
            str = cell2str(obj.list);
        end
        
        % Specialised method to return an iterator array for this list array.
        % 2010-Jul-27: Modified to return array of ImagingListIterators of
        % the same dimensionality as the array of ImagingLists.
        function iter = createIterator(obj)
            % Pre-allocate array of ImagingListIterators to empty
            % ImagingLists.
            import mlpatterns.*;
            iter = repmat(ImagingListIterator(),size(obj));
            
            % Use linear index to populate ImagingListIterator array.
            for i = 1:numel(obj)
                iter(i) = ImagingListIterator(obj(i));
            end
        end
    end % methods
end % classdef
