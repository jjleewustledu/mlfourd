classdef FourdfpInfo < handle & mlfourd.Analyze75Info
	%% FOURDFPINFO relies on Jimmy Shen's mlniftitools to obtain header information from 4dfp files,
    %  restructuring for internal representation as NIfTI whenever possible.  It also manages 
    %  ifh and img.rec files needed by 4dfp.
    ...
    properties (Constant) 
        ...
        SUPPORTED_EXT = {'.4dfp.hdr' '.4dfp.ifh' '.4dfp.img' '.4dfp.img.rec'};
    end
	...
	methods 
        ...
        function g = get.ifh_orientation(this)
            ...
            error('mlfourd:ValueError', 'FourdfpInfo.get.ifh_orientation');
        end 
        ...
        function ana = ensureLoadingOrientation(~, ana)
            ...
            %if isfield(ana.hdr.hist, 'sform_code') && ana.hdr.hist.sform_code > 0
            %    S = [ana.hdr.hist.srow_x; ana.hdr.hist.srow_y; ana.hdr.hist.srow_z];
            %    S = abs(S);
            %    S = S(:, 1:3);
        ...
        function save(this)
            ...
            try                
                ...
                this.imgrec.add('mlfourd.FourdfpInfo.save %s', this.fqfileprefix);
                this.imgrec.save;
    ...
    methods (Access = private)
        ...
        function tf   = consistentOrientation(this, o)
            switch (lower(this.Orientation(1:7)))
                ...
                case 'sagitta'
                    tf = (4 == o);
                otherwise
                    error('mlfourd:unsupportedSwitchcase', 'FourdfpInfo.assertOrientation');
            end
        end    
        function p    = ifhread(this)
            import mlfourdfp.*;
            p = IfhParser.load(strcat(this.fqfileprefix, '.4dfp.ifh'), 'N', this.N);
            s = struct(p);
            
            %% possibly breaking pipeline
            % assert(lstrfind(lower(this.ImgDataType), lower(s.number_format)), ...
            %     'mlfourd:inconsistentParamValue', ...
        ...
        function [cc,sr] = imgrecread(this)
            p = mlfourdfp.ImgRecParser.load(strcat(this.fqfileprefix, '.4dfp.img.rec'));
            cc = p.cellContents;
            sr = p.commonSform;
        end
        function ii   = initialIfh(this)
            if (~isfile(strcat(this.fqfileprefix, '.4dfp.ifh')))
