classdef NIfTIdecorator5 < mlfourd.INIfTI & mlio.IOInterface
    ...
    %  KLUDGE:  Matlab checks that dependent properties do not access stored values.  Multiple composed NIfTIdecorator5 
    ...
    %      In class 'mlfourd.NIfTIdecorator5', the get method for Dependent property 'filename' attempts to 
    %      access the stored property value.  Dependent properties don't store a value and can't be accessed
    %      from their get method.
    %     
    %      Error in mlfourd.NIfTIdecorator5/get.filename (line 66)
    %                  fn = this.component_.filename;
    %     
    %  Using NIfTIdecorator5/3/4/5 for each decorator subclass is a work-around.
    ...
    methods (Static)
        ...
        function this = load(varargin)
            ...
            %  Usage:  obj = NIfTIdecorator5.load(filename[, description]);
            
            import mlfourd.*;
            this = NIfTIdecorator5(NIfTId.load(varargin{:}));
        end
    end

    methods
 		function this = NIfTIdecorator5(varargin) 
            ...
            %  Usage:  obj = NIfTIdecorator5(INIfTI_object);
