classdef NIfTIdecorator6 < mlfourd.INIfTI & mlio.IOInterface
    ...
    %  KLUDGE:  Matlab checks that dependent properties do not access stored values.  Multiple composed NIfTIdecorator6 
    ...
    %      In class 'mlfourd.NIfTIdecorator6', the get method for Dependent property 'filename' attempts to 
    %      access the stored property value.  Dependent properties don't store a value and can't be accessed
    %      from their get method.
    %     
    %      Error in mlfourd.NIfTIdecorator6/get.filename (line 66)
    %                  fn = this.component_.filename;
    %     
    %  Using NIfTIdecorator6/3/4/5 for each decorator subclass is a work-around.
    ...
    methods (Static)
        ...
        function this = load(varargin)
            ...
            %  Usage:  obj = NIfTIdecorator6.load(filename[, description]);
            
            import mlfourd.*;
            this = NIfTIdecorator6(NIfTId.load(varargin{:}));
        end
    end

    methods
 		function this = NIfTIdecorator6(varargin) 
            ...
            %  Usage:  obj = NIfTIdecorator6(INIfTI_object);
