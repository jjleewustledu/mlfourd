classdef BlurringNIfTI < mlfourd.AbstractNIfTIDecorator
	%% BlurringNIfTI is a concrete decorator.  Blur must be provided as fwhh in mm.
    ...
    methods (Static)       
        function width = sigma2width(sigma, fheight)
            ...
            switch (nargin)
                ...
                case 2 
                otherwise
                    error('mlfourd:InputParamsErr', help('BlurringNIfTI.sigma2width'));
            end
        ...
        function sigma = width2sigma(width, fheight)
            ...
            switch (nargin)
                ...
                case 2
                otherwise
                    error('mlfourd:InputParamsErr', help('BlurringNIfTI.width2sigma'));
            end
    ...
    methods 
        ...
        %%
 		...
 		function this = BlurringNIfTI(cmp, varargin)
 			%% BLURRINGNIFTI 
 			%  @param cmp is an mlfourd.INIfTIComponent
            %  @param blur is numeric, default [], fwhh in mm
            %  @param mask is numeric or mlfourd.INIfTI

            this = this@mlfourd.AbstractNIfTIDecorator(cmp);
            this = this.append_descrip('decorated by BlurringNIfTI');
        ...
        function obj  = clone(this)
            obj            = mlfourd.BlurringNIfTI(this.component.clone);
        ...
        function fp   = blurredFileprefix(this)
            fp = sprintf('%s_b%g', this.fileprefix, round(10*max(this.blur)));
        end
        function tf   = isequaln(this, bnii)
            tf = isa(bnii, class(this));
            if (tf)
                tf = isa(bnii, 'mlfourd.BlurringNIfTI');
                if (tf)
            ...
            if (~isequaln(this.mask_, bnii.mask))
                tf  = false;
                warning('mlfourd:notIsequaln', 'BlurringNIfTI.isequaln:  found mismatch at this.mask.');
                return
            end
            if (~isequaln(this.blur_, bnii.blur))
                tf  = false;
                warning('mlfourd:notIsequaln', 'BlurringNIfTI.isequaln:  found mismatch at this.blur.');
                return
            end
            if (~isequaln(this.blurCount_, bnii.blurCount))
                tf  = false;
                warning('mlfourd:notIsequaln', 'BlurringNIfTI.isequaln:  found mismatch at this.blurCount.');
                return
            end
        end
    end 
    ...
    methods (Static, Access = private)
        function img   = gaussFullwidth(img, width, metric, metppix, height)
            ...
            %                 numeric objects.
            %  Usage: gimg = BlurringNIfTI.gaussFullwidth(img, width, metric, metppix, height)
            %         img:       numeric object
            ...
            %  See also:  BlurringNIfTI.gaussSigma
            ...
            switch (nargin)
                ...
                otherwise
                    error('mlfourd:NotImplementedErr', ...
                         ['BlurringNIfTI.gaussFullwidth.nargin->' num2str(nargin)]);
            end
            img = BlurringNIfTI.gaussSigma(img, BlurringNIfTI.width2sigma(width, height), metric, metppix);
        end 
        function img   = gaussSigma(img, sigma, metric, metppix)
            %% GAUSSSIGMA applies multi-dimensional, anisotropic, Gaussian filtering to 
            %             numeric objects.
            %  Usage: gimg = BlurringNIfTI.gaussSigma(img, width, metric, metppix)
            %         img:       numeric object
            ...
            switch (nargin)
                ...
                otherwise
                    error('mlfourd:NotImplementedErr', ...
                         ['BlurringNIfTI.gaussSigma.nargin->' num2str(nargin)]);
            end
            switch (lower(metric)) 
                ...
                otherwise
                    error('mlfourd:NotImplementedErr', ...
                         ['BlurringNIfTI.gaussFullwidth.metric->' metric ' was unrecognizable;\n' ...
                          'try pixel(s), voxel(s), mm, cm']);
            end            
            img     = double(img);
            imgRank = length(size(img));
            if (length(metppix) < length(sigma))
                metppix = BlurringNIfTI.stretchVec(metppix, length(sigma));
            ...
            krnlLens = BlurringNIfTI.KERNEL_MULTIPLE*ceil(2*sigma)+1; % see also imgaussfilt, imgaussfilt3
        ...
        function sz    = embedVecInSitu(sz, fixedsz)
            ...
            %  e.g.  >> sz = BlurringNIfTI.embedVecInSitu([2 2 30], [18 18 31 100])
            %        sz = 
            %            2 2 30 100
            ...
            switch (nargin)
                ...
                otherwise
                    error('mlfourd:InputParamsErr:InputParamsErr', ...
                         ['BlurringNIfTI.stretchVec.nargin->' num2str(nargin)]);
            end
