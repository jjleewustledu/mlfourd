classdef RoiBuilder
    ...
    methods (Static)
        ...
        function   binnii = threshRoiByStd(fpmsk, numstd)
            ...
            %  Usage:  binnii = mlfourd.RoiBuilder.threshRoiByStd(fpmsk, numstd)
            ...
            switch (nargin)
                ...
                otherwise
                    error('mlfourd:InputParamsErr', ...
                         ['RoiBuilder.threshRoiByStd does not support ' num2str(nargin) ' input params']);
            end
            bin    = double(fpmsk.img > thresh);
            binnii = fpmsk.makeSimilar(bin, 'RoiBuilder.threshRoiByStd'); 
            disp(['RoiBuilder.threshRoiByStd.maxfp -> ' num2str(maxfp)]);
        ...
        function   binnii = threshRoiByCoverage(fpmsk, cover)
            ...
            %  Usage:  binnii = mlfourd.RoiBuilder.threshRoiByCoverage(fpmsk, cover)
            %          numstd:  set threshold to be max intensity - numstd*std (optional)
            %                   otherwise mask coverage will be set to 1 percent of voxels
            %  OBSOLETE, DEPRECATED
            mlfourd.NIfTI.isNIfTI
            ...
            if (nargin < 1 || nargin > 2)
                error('mlfourd:InputParamsErr', ...
                     ['RoiBuilder.threshRoiByStd does not support ' ...
        ...
        function   nii    = makeArteryRoi(epinii, fgnii)
            ...
            %  Usage:  nii = mlfourd.RoiBuilder.makeArteryRoi(epinii, fgnii)
            %  Requires:  diplib
            %
            ...
            nii      = fgnii.makeSimilar( ...
                       arteries, 'from RoiBuilder.make_arteriesRois');
        ...
        function            prepareMatrices(bldr)
            ...
            flirtf.invertxfm(iopts);
            copyfile(       matFqfn(tpth, 'bt1_rot', ['b' RoiBuilder.EP2D '_rot_mcf_meanvol.mat']),  ...
        ...
        function            prepareRoisOnT1(bldr)
            ...
            rois   = RoiBuilder.ROIS;
            flirtf = FlirtFacade(bldr);
            msg    = '';
            for r = 2:numel(rois) %#ok<*FORFLG>
                try
                    opts.ref  = flirtf.t1;
                    opts.in   = fullfile(flirtf.atlasPath, RoiBuilder.ATLASES{r});
                    opts.out  = RoiBuilder.onT1(rois{r});
                    opts.init = fullfile(bldr.transformationsPath, ['MNI' mlfsl.FlirtFacade.FLIRT_TOKEN 't1.mat']);
                    [~,msg]   = flirtf.applyxfm(opts);
                catch ME
                    handexcept(ME, msg)
                end
            end
        ...
        function            prepareModelok(bldr)
            ...
            try
                 opts.ref  = flirtf.t1;
                 opts.in   = 'modelok';
                 opts.out  = RoiBuilder.onT1('modelok');
        ...
        function            registerToModalities(bldr)
            ...
            import mlfourd.* mlfsl.*;
            modes  = RoiBuilder.MODES;            
            rois   = RoiBuilder.ROIS;  %#ok<*PROP>
            flirtf = FlirtFacade(bldr);
            msg    = '';
            for r = 1:length(rois) %#ok<PROP>
                if (strcmp(rois{r}, 'white'))
                    trimroi(RoiBuilder.onT1(rois{r}), ...
                           {RoiBuilder.onT1(rois{4}) RoiBuilder.onT1(rois{5})});
                end
                for m = 1:length(modes)
                    try
                        [~,msg] =         copyfile(RoiBuilder.t1ToMode(          modes{m}), ...
                                                   RoiBuilder.roiToMode(rois{r}, modes{m}), 'f'); %#ok<PROP>                         
                        opts.ref  = RoiBuilder.theMetrics{m};
                        opts.in   = RoiBuilder.onT1(     rois{r});
                        opts.out  = RoiBuilder.roiOnMode(rois{r}, modes{m});
                        opts.init = RoiBuilder.inits(    rois{r}, modes{m});
        ...
        function ms = theMetrics
            import mlfourd.*;
            ms = cell(size(RoiBuilder.MODES));
            for m = 1:length(RoiBuilder.MODES)
                ms{m} = ['cbf_' RoiBuilder.MODES{m}];
            end
        end        
        ...
        function [metrics, modelOks, parenchymas, mcas, grays, deepgrays, whites, lefts, rights, Cerebellums] = ...
            ...
            import mlfourd.*; 
            fprintf('RoiBuilder.tabulateModalRois:\n');            
            
            %% prepare all inputs as RoiFactories
            metrics     = RoiBuilder.toNIfTI(metrics);
            modelOks    = RoiBuilder.toNIfTI(modelOks);
            parenchymas = RoiBuilder.toNIfTI(parenchymas);
            mcas        = RoiBuilder.toNIfTI(mcas);
            grays       = RoiBuilder.toNIfTI(grays);
            deepgrays   = RoiBuilder.toNIfTI(deepgrays);
            whites      = RoiBuilder.toNIfTI(whites);
            lefts       = RoiBuilder.toNIfTI(lefts);
            rights      = RoiBuilder.toNIfTI(rights);            
            Cerebellums = RoiBuilder.toNIfTI(Cerebellums);
            
            for m = 1:length(grays)
                grays{m} = grays{m} + whites{m};
                gtone    = grays{m} > 1;
                grays{m} = grays{m} .* (~gtone) + gtone;
                grays{m}.fileprefix = ['cortical' mlfsl.FlirtFacade.FLIRT_TOKEN RoiBuilder.MODES{m}];
                grays{m}.save;
            end
            ...
            for m = 1:nModes 
                ...
                metricFactories{m} = RoiBuilder(metrics{m}, parenchymas{m} .* mcas{m} .* rights{m} .* whites{m});
                switch (m)
                    case 1
            ...
            for m = 1:nModes
                ...
                fg.fileprefix  = 'fg';
                modelokFactories{m} = RoiBuilder(metrics{m}, modelOks{m} .* parenchymas{m});
            ...
            for m = 1:nModes
                fg             = modelOks{m} .* parenchymas{m} .* mcas{m};
                fg.fileprefix  = 'fg';
                mcaFactories{m} = RoiBuilder(metrics{m}, mcas{m} .* modelOks{m});
            ...
            for m = 1:nModes
                fg             = modelOks{m} .* parenchymas{m} .* mcas{m};
                fg.fileprefix  = 'fg';
                fgFactories{m} = RoiBuilder(metrics{m}, fg);
            ...
            fprintf('modelOks:');
            for m = 1:nModes
                ipsiFactories{m} = RoiBuilder(metrics{m}, lefts{m});
            ...
            fprintf('modelOks:');
            for m = 1:nModes
                contraFactories{m} = RoiBuilder(metrics{m}, rights{m});
        ...
        function nii = toNIfTI(fp)
            ...
            if (iscell(fp))
                ...
                elseif (isa(fp, 'mlfourd.ImageInterface'))
                    nii = NIfTI(fp);
                elseif (isa(fp, 'mlfourd.RoiBuilder'))
    ...
    methods (Access = 'protected')
        ...
        function this = RoiBuilder(metric, rois)
            
            %% CTOR
            %  Usage: this = mlfourd.RoiBuilder(metric [, rois])
            %         ^ Builder                 ^         ^ fileprefixes or NIfTIs
            import mlfourd.*;
            if (nargin > 0)
                this.theMetric = RoiBuilder.toNIfTI(metric);
            end
            if (nargin > 1)
                        rois = ensureCell(        rois);
                this.theRois = RoiBuilder.toNIfTI(rois);
            end
        end % ctor
        ...
        function this = load(this, object)
            ...
            for o = 1:length(object)
                this.theRois{nNiis0 + o} = RoiBuilder.toNIfTI(object{o});
            end
        end % load        
        function this = restrictAllRois(this, fg)
            
            import mlfourd.*;
            fg = RoiBuilder.toNIfTI(fg);
        ...
        function this = intersectNbyM(this, miis)
            ...
            for n = 1:length(this.theRois)
                for m = 1:length(miis)
                    miis{m} = RoiBuilder.toNIfTI(miis{m});
                    tmp = this.theRois{n};
                    tmp = miis{m} .* tmp;
