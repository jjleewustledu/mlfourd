classdef Test_NIfTId < matlab.unittest.TestCase
	...
	methods (Test)
        ...
        function test_untouch(this)
            ...
            obj = mlfourd.NIfTId(this.testObj);
            obj.noclobber = true;
        ...
        function test_negentropy(this)
            this.verifyEqual(this.testObj.negentropy, -2.015574498248682e-05, 'RelTol', 1e-3);
        end        
        ...
        function test_ctorStruct(this)
            ...
            warning('off', 'mlfourd:mismatchedField');
            this.verifyEqual(mlfourd.NIfTId(s), this.testObj);
            warning('on',  'mlfourd:mismatchedField');
        end
        ...
        function test_fov(this)
            this.verifyEqual(this.testObj.fov, [256  256  130], 'RelTol', 1e-3);
        end
        ...
        function test_entropy(this)
            this.verifyEqual(this.testObj.entropy, 2.015574498248682e-05, 'RelTol', 1e-3);
        end
        ...
        function test_img(this)
            this.verifyEqual(this.testObj.img(64,64,32), single(318), 'RelTol', 1e-3);
 	...
 	methods (TestMethodSetup)
		function setupNIfTIdTest(this)
            this.pwd0 = pushd(this.fslPath);
 			this.testObj = mlfourd.NIfTId(this.testObj_); 
 		end
    end
    ...
    methods (TestMethodTeardown)
        function cleanupFiles(this)
            ...
            this.deleteExisting(fullfile(this.fslPath, '*.nii'));
            this.deleteExisting(fullfile(this.fslPath, 'Test_NIfTId*'));
            popd(this.pwd0);
        end        
    end
    ...
    methods (Access = private)
        function this = ensureFiles(this)
            ...
            assert(isfolder(this.sessionPath));
        ...
        function verify001_4dfp(this, niid, varargin)
            ...
            this.verifyEqual(niid.hdr.dime.pixdim, [1 1 1 1 1 1 1 1], 'RelTol', 1e-3);
            ...
            this.verifyEqual(niid.hdr.hist.sform_code, 1);
            this.verifyEqual(niid.hdr.hist.quatern_b, 0, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.quatern_c, 0, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.quatern_d, 0, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_x, -87, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_y, -123, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_z, -127, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_x, [1 0 0 -87], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_y, [0 1 0 -123], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_z, [0 0 1 -127], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.originator, [88 124 128], 'RelTol', 1e-3)
            this.verifyEqual(niid.machine, 'ieee-le');
            this.verifyEqual(sum(sum(sum(niid.img))), single(1128125696), 'RelTol', 1e-3);
        end
        function verify001(this, niid, varargin)
            ...
            this.verifyEqual(niid.hdr.dime.pixdim, [-1 1 1 1 2.400000095367432 1 1 1], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.dime.vox_offset, 352);
            this.verifyEqual(niid.hdr.dime.xyzt_units, 10);
            this.verifyEqual(niid.hdr.dime.glmax, 1296);
            this.verifyEqual(niid.hdr.dime.glmin, 0);
            this.verifyEqual(niid.hdr.hist.qform_code, 1);
            this.verifyEqual(niid.hdr.hist.sform_code, 1);
            this.verifyEqual(niid.hdr.hist.quatern_b, -0.492527604103088, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.quatern_c,  0.492527604103088, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.quatern_d, -0.507362365722656, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_x,  83.664199829101562, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_y,  1.656464843750000e+02, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_z,  1.221617279052734e+02, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_x, [0 0.029666258022189 -0.999559879302979 83.664199829101562], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_y, [-1 0 0 1.656464843750000e+02], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_z, [0 -0.999559879302979 -0.029666244983673 1.221617279052734e+02], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.originator, [124 128 88], 'RelTol', 1e-3)
            this.verifyEqual(niid.machine, 'ieee-le');
        ...
        function verifyt1_dcm2niix_4dfp(this, niid, varargin)
            ...
            this.verifyEqual(niid.hdr.dime.datatype, 16);
            this.verifyEqual(niid.hdr.dime.bitpix, 32);
            this.verifyEqual(niid.hdr.dime.pixdim, [1 0.999997019767761 1 1 1 1 1 1], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.dime.vox_offset, 352);
            this.verifyEqual(niid.hdr.dime.xyzt_units, 10);
            this.verifyEqual(niid.hdr.dime.glmax, 1296);
            this.verifyEqual(niid.hdr.dime.glmin, 0);
            ...
            this.verifyEqual(niid.hdr.hist.quatern_c, 0, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.quatern_d, 0, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_x, -86.999478460139812, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_y, -123, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_z, -127, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_x, [0.999997019767761 0 0 -86.999478460139812], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_y, [0 1 0 -123], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_z, [0 0 1 -127], 'RelTol', 1e-3);
        ...
        function verifyt1_dcm2niix(this, niid, varargin)
            ...
            this.verifyEqual(niid.hdr.hist.sform_code, 1);
            this.verifyEqual(niid.hdr.hist.quatern_b, 0, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.quatern_c, -0.014834761619568, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.quatern_d, 0, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_x, -83.693443298339844, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_y, -81.353515625000000, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.qoffset_z, -1.379176177978516e+02, 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_x, [0.999557375907898 0 -0.029666258022189 -83.693443298339844], 'RelTol', 1e-3);
            this.verifyEqual(niid.hdr.hist.srow_y, [0 1 0 -81.353515625000000], 'RelTol', 1e-3);
        ...
        function verifyEqualNIfTId(this, n1, n2)
            toignore = [mlfourd.NIfTId.EQUALN_IGNORES ...
                ...
                'filetype' 'hdxml' 'imagingInfo' 'orient'}];
            props = properties(n1);            
