classdef Test_NIfTIContext < matlab.unittest.TestCase
	%% TEST_NIFTICONTEXT 

	%  Usage:  >> results = run(mlfourd_unittest.Test_NIfTIContext)
 	%          >> result  = run(mlfourd_unittest.Test_NIfTIContext, 'test_dt')
	...
	methods (Test)
        function test_loadNoExtension(this)
            import mlfourd.*;
            niih = NIfTIContext(myfileprefix(this.ref.dicomAsNiigz));
        ...
        function test_mutateInnerImagingFormatByFilesuffix(this) 
            ...
            niih_ = NIfTIContext(this.ref.dicomAsNiigz);   
        ...
        function test_load_t1niigz(this) 
 			import mlfourd.*;           
            niih = NIfTIContext(this.ref.dicomAsNiigz); 
            this.verifyEqual(class(niih.img), 'int16');
            this.verifyEqual(size(niih.img),  [176 248 256]);
            this.verifyEqual(niih.filesuffix, '.nii.gz');
            
            tmp = tempFqfilename(fullfile(pwd, 'test_load_t1niigz'));
            niih.saveas([tmp '.nii.gz']);
            this.verifyTrue(lexist([tmp '.nii.gz'], 'file'));
            this.deleteExisting([tmp '.nii.*']);
            if (this.doview); niih.fsleyes; end
        end
        function test_load_t14dfp(this)
 			import mlfourd.*;
            niih = NIfTIContext(this.ref.dicomAsFourdfp, 'circshiftK', 0, 'N', true); % sagittal            
            this.verifyEqual(class(niih.img), 'single');
            this.verifyEqual(size(niih.img),  [176 248 256]);
            this.verifyEqual(niih.filesuffix, '.4dfp.hdr');
...
%             this.verifyEqual(niih.machine, 'ieee-le');   
            ...
            tmp = tempFqfilename(fullfile(pwd, 'test_load_t14dfp'));
        ...
        function test_load_001mgz(this)
 			import mlfourd.*;
            niih = NIfTIContext(this.ref.surferAsMgz); % sagittal            
            this.verifyEqual(class(niih.img), 'int16');
            this.verifyEqual(size(niih.img),  [248 256 176]);
            this.verifyEqual(niih.filesuffix, '.nii.gz');
            
            tmp = tempFqfilename(fullfile(pwd, 'test_load_001mgz'));
            niih.saveas([tmp '.mgz']);
        ...
        function test_load_001niigz(this)
 			import mlfourd.*;
            niih = NIfTIContext(this.ref.surferAsNiigz); % sagittal            
            this.verifyEqual(class(niih.img), 'int16');
            this.verifyEqual(size(niih.img),  [248 256 176]);
            this.verifyEqual(niih.filesuffix, '.nii.gz');
        ...
        function test_load_0014dfp(this) 
 			import mlfourd.*;
            niih = NIfTIContext(this.ref.surferAsFourdfp, 'circshiftK', 0, 'N', true); % sagittal            
            this.verifyEqual(class(niih.img), 'single');
            this.verifyEqual(size(niih.img),  [176 248 256]);
            this.verifyEqual(niih.filesuffix, '.4dfp.hdr');
        ...
        function test_load_ana001(this) 
 			import mlfourd.*;
            niih = NIfTIContext('ana001.hdr', 'circshiftK', 0, 'N', false); % sagittal            
            this.verifyEqual(class(niih.img), 'int16');
            this.verifyEqual(size(niih.img),  [176 248 256]);
        ...
        function test_load_T1mgz(this)
 			import mlfourd.*;
            niih = NIfTIContext(this.ref.T1AsMgz); % sagittal            
        ...
        function test_load_T1niigz(this)
 			import mlfourd.*;            
            niih = NIfTIContext(this.ref.T1AsNiigz); % sagittal            
            this.verifyEqual(class(niih.img), 'uint8');
            this.verifyEqual(size(niih.img),  [256 256 256]);
            this.verifyEqual(niih.filesuffix, '.nii.gz');
        ...
        function test_load_T14dfp(this) 
 			import mlfourd.*;            
            niih = NIfTIContext(this.ref.T1AsFourdfp, 'circshiftK', 0, 'N', true); % sagittal            
            this.verifyEqual(class(niih.img), 'single');
            this.verifyEqual(size(niih.img),  [256 256 256]);
            this.verifyEqual(niih.filesuffix, '.4dfp.hdr');
